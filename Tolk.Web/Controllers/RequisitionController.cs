    ...
    public class RequisitionController : Controller
    {
        ...
        public async Task<IActionResult> ListRequisitions(IDataTablesRequest request)
        {
            ...
            return AjaxDataTableHelper.GetData(request, requisitions.Count(), model.Apply(requisitions), x => x.Select(r => new RequisitionListItemModel
            {
                ...
                OrderDateAndTime = r.Request.Order.ExpectedLength.HasValue ? $"{r.Request.RespondedStartAt:yyyy-MM-dd HH:mm}-{r.Request.RespondedStartAt.Value.Add(r.Request.Order.ExpectedLength.Value):HH:mm}" : $"{r.Request.Order.StartAt:yyyy-MM-dd HH:mm}-{r.Request.Order.EndAt:HH:mm}",
                Status = r.Status,
