        ...
        public void OrderGroupCancelledByCustomer(RequestGroup requestGroup)
        {
            ...
            if (email != null)
            {
                ...
                CreateEmail(email.ContactInformation, $"Avbokad sammanhållen bokningsförfrågan boknings-ID {orderNumber}",
                    ...
                    requestGroup.Ranking.FrameworkAgreement.AgreementNumber,
                    true);
            }
...
<div>{GoToRequestButton(oldRequest.RequestId, textOverride: "Gå till ursprungligt uppdrag", autoBreakLines: false)}</div>";
                CreateEmail(
                     ...
                     replacementRequest.Ranking.FrameworkAgreement.AgreementNumber,
                     true);
            }
            ...
            if (webhook != null)
            {
                CreateWebHookCall(
                    ...
                    {
                        OriginalRequest = await GetRequestModel<RequestModel>(oldRequest),
                        ReplacementRequest = await GetRequestModel<RequestModel>(await GetRequest(replacementRequest.RequestId))
                    },
                   webhook.ContactInformation,
                   NotificationType.RequestReplacementCreated,
                   webhook.RecipientUserId
        ...
        {
            NullCheckHelper.ArgumentCheckNull(request, nameof(RequestNeedsFullAnswerCreated), nameof(NotificationService));
            ...
            if (answerLevel != RequiredAnswerLevel.Full)
            {
...
<div>{GoToRequestButton(request.RequestId, textOverride: "Till förfrågan", autoBreakLines: false)}</div>";
            ...
            {
                CreateWebHookCall(
                    await GetRequestModel<RequestModel>(request),
                    webhook.ContactInformation,
        ...
        {
            NullCheckHelper.ArgumentCheckNull(request, nameof(RequestNeedsAcceptanceCreated), nameof(NotificationService));
            var order = request.Order;
            var frameworkAgreementNumber = request.Ranking.FrameworkAgreement.AgreementNumber;
            var answerLevel = EnumHelper.Parent<RequestAnswerRuleType, RequiredAnswerLevel>(request.RequestAnswerRuleType);
            if (answerLevel != RequiredAnswerLevel.Acceptance)
            {
                throw new NotSupportedException();
            }
...
<div>{GoToRequestButton(request.RequestId, textOverride: "Till förfrågan", autoBreakLines: false)}</div>";
        ...
        {
            NullCheckHelper.ArgumentCheckNull(request, nameof(FlexibleRequestCreated), nameof(NotificationService));
            var order = request.Order;
            var frameworkAgreementNumber = request.Ranking.FrameworkAgreement.AgreementNumber;
            var answerLevel = EnumHelper.Parent<RequestAnswerRuleType, RequiredAnswerLevel>(request.RequestAnswerRuleType);
            //TODO: Verify that the notification is sent to a request that needs full answer or acceptance!
            if (answerLevel != RequiredAnswerLevel.Acceptance && answerLevel != RequiredAnswerLevel.Full)
            {
                throw new NotSupportedException();
            }
            var email = GetOrganisationNotificationSettings(request.Ranking.BrokerId, NotificationType.FlexibleRequestCreated, NotificationChannel.Email);
            if (email != null)
            {
                string bodyPlain = $"Bokningsförfrågan för tolkuppdrag {order.OrderNumber} från {order.CustomerOrganisation.Name} har inkommit via {Constants.SystemName}.\nMyndighetens organisationsnummer: {order.CustomerOrganisation.OrganisationNumber}\nMyndighetens Peppol-ID: {order.CustomerOrganisation.PeppolId}\n\nObservera att alla svar kopplat till förfrågan måste lämnas via avropstjänsten.\n\n" +
                    "Detta är en förfrågan med flexibel starttid\n" +
                    //Add this text, if the request needs only acceptance
                    (answerLevel == RequiredAnswerLevel.Acceptance ? "Denna bokningsförfrågan behöver endast bekräftas i ett första steg, där bekräftelsen måste innehålla acceptans av alla krav kopplade till bokningsförfrågan. Namngivning av tolk kan göras senare. \n" : string.Empty) +
                    $"\tUppdragstyp: {EnumHelper.GetDescription(order.AssignmentType)}\n" +
                    $"\tRegion: {order.Region.Name}\n" +
                    $"\tSpråk: {order.OtherLanguage ?? order.Language?.Name}\n" +
                    ...
                    $"\tFörväntad längd på uppdraget: {order.ExpectedLength?.ToSwedishString("hh\\:mm")}\n" +
                ...
                string bodyHtml = $@"Bokningsförfrågan för tolkuppdrag {order.OrderNumber} från {order.CustomerOrganisation.Name} har inkommit via {Constants.SystemName}.<br />Myndighetens organisationsnummer: {order.CustomerOrganisation.OrganisationNumber}<br />Myndighetens Peppol-ID: {order.CustomerOrganisation.PeppolId}<br /><br />Observera att alla svar kopplat till förfrågan måste lämnas via avropstjänsten.<br />
Detta är en förfrågan med flexibel starttid<br />";
                bodyHtml += (answerLevel == RequiredAnswerLevel.Acceptance ? "Denna bokningsförfrågan behöver endast bekräftas i ett första steg, där bekräftelsen måste innehålla acceptans av alla krav kopplade till bokningsförfrågan. Namngivning av tolk kan göras senare.<br />" : string.Empty);
...
<li>Förväntad längd på uppdraget: {order.ExpectedLength?.ToSwedishString("hh\\:mm")}</li>";
...
<div>{GoToRequestButton(request.RequestId, textOverride: "Till förfrågan", autoBreakLines: false)}</div>";
            ...
            var webhook = GetOrganisationNotificationSettings(request.Ranking.BrokerId, NotificationType.FlexibleRequestCreated, NotificationChannel.Webhook);
            if (webhook != null)
            {
                CreateWebHookCall(
                    await GetFlexibleRequestModel(request),
                    webhook.ContactInformation,
                    webhook.NotificationType,
                    webhook.RecipientUserId
                );
        ...
        public async Task RequestGroupCreated(RequestGroup requestGroup)
        {
            ...
            var answerLevel = EnumHelper.Parent<RequestAnswerRuleType, RequiredAnswerLevel>(requestGroup.RequestAnswerRuleType);
            switch (answerLevel)
            {
                ...
                case RequiredAnswerLevel.Acceptance:
                    {
                        ...
                        if (email != null)
                        {
                            string bodyPlain = $"Bokningsförfrågan för ett sammanhållet tolkuppdrag {orderGroup.OrderGroupNumber} från {orderGroup.CustomerOrganisation.Name} har inkommit via {Constants.SystemName}.\nMyndighetens organisationsnummer: {orderGroup.CustomerOrganisation.OrganisationNumber}\nMyndighetens Peppol-ID: {orderGroup.CustomerOrganisation.PeppolId}\n\nObservera att bekräftelse måste lämnas via avropstjänsten.\n\n" +
                                ...
                                $"\tTillfällen: \n" +
                                $"{GetOccurences(orderGroup.Orders)}\n" +
                                $"\tBekräfta senast: {requestGroup.LastAcceptAt?.ToSwedishString("yyyy-MM-dd HH:mm")}" +
                                $"\tTillsätt tolk senast: {requestGroup.ExpiresAt?.ToSwedishString("yyyy-MM-dd HH:mm")}" +
                                GoToRequestGroupPlain(requestGroup.RequestGroupId);
                            string bodyHtml = $@"Bokningsförfrågan för ett sammanhållet tolkuppdrag {orderGroup.OrderGroupNumber} från {orderGroup.CustomerOrganisation.Name} har inkommit via {Constants.SystemName}.<br />Myndighetens organisationsnummer: {orderGroup.CustomerOrganisation.OrganisationNumber}<br />Myndighetens Peppol-ID: {orderGroup.CustomerOrganisation.PeppolId}<br /><br />Observera att bekräftelse måste lämnas via avropstjänsten.<br />
                    Denna bokningsförfrågan behöver endast bekräftas i ett första steg, där bekräftelsen behöver innehålla acceptans av alla krav kopplade till bokningsförfrågan. Namngivning av tolk kan göras senare. <br />
                    <ul>
                    ...
                    <li>Region: {orderGroup.Region.Name}</li>
                    <li>Språk: {orderGroup.LanguageName}</li>
                    <li>Bekräfta senast: {requestGroup.LastAcceptAt?.ToSwedishString("yyyy-MM-dd HH:mm")}</li>
                    <li>Tillsätt tolk senast: {requestGroup.ExpiresAt?.ToSwedishString("yyyy-MM-dd HH:mm")}</li>
                    </ul>
                    <div>{GoToRequestGroupButton(requestGroup.RequestGroupId, textOverride: "Till förfrågan", autoBreakLines: false)}</div>";
                            CreateEmail(
                                email.ContactInformation,
                                $"Ny sammanhållen bokningsförfrågan registrerad som kräver initial bekräftelse: {orderGroup.OrderGroupNumber}",
                                bodyPlain,
                                bodyHtml,
                                email.NotificationType,
                                frameworkAgreementNumber,
...
Notera att er förfrågan INTE skickas vidare till nästa förmedling, tills dess sista svarstid är satt.";
        ...
        public void RequestGroupCreatedWithoutExpiry(RequestGroup newRequestGroup)
        {
            ...
            var frameworkAgreementNumber = newRequestGroup.Ranking.FrameworkAgreement.AgreementNumber;
            if (NotficationTypeAvailable(notificationType, NotificationConsumerType.Customer, NotificationChannel.Email) && !NotficationTypeExcludedForCustomer(notificationType))
            {
...
Notera att er förfrågan INTE skickas vidare till nästa förmedling, tills dess sista svarstid är satt.";
        ...
        public void RequestAnswerDenied(Request request)
        {
            ...
            string orderNumber = request.Order.OrderNumber;
            string frameworkAgreementNumber = request.Ranking.FrameworkAgreement.AgreementNumber;
        ...
        {
            NullCheckHelper.ArgumentCheckNull(request, nameof(RequestExpiredDueToNotFullyAnswered), nameof(NotificationService));
            var orderNumber = request.Order.OrderNumber;
            string frameworkAgreementNumber = request.Ranking.FrameworkAgreement.AgreementNumber;
            var email = GetOrganisationNotificationSettings(request.Ranking.BrokerId, NotificationType.RequestLostDueToNotFullyAnswered, NotificationChannel.Email);
            if (email != null)
            {
                CreateEmail(email.ContactInformation,
                    $"Bokningsförfrågan {orderNumber} har gått vidare till nästa förmedling i rangordningen",
                    $"Ni har inte namngivit tolk för bekräftad bokningsförfrågan {orderNumber} från {request.Order.CustomerOrganisation.Name} organisationsnummer {request.Order.CustomerOrganisation.OrganisationNumber}.\nTidsfristen enligt ramavtal har nu gått ut. {GoToRequestPlain(request.RequestId)}",
                    $"Ni har inte namngivit tolk för bekräftad bokningsförfrågan {orderNumber} från {request.Order.CustomerOrganisation.Name} organisationsnummer {request.Order.CustomerOrganisation.OrganisationNumber}.<br />Tidsfristen enligt ramavtal har nu gått ut. {GoToRequestButton(request.RequestId)}",
                    NotificationType.RequestLostDueToNotFullyAnswered,
                    frameworkAgreementNumber,
                    true);
            }
            var webhook = GetOrganisationNotificationSettings(request.Ranking.BrokerId, NotificationType.RequestLostDueToNotFullyAnswered, NotificationChannel.Webhook);
            if (webhook != null)
            {
                CreateWebHookCall(
                    new RequestLostDueToInactivityModel
        ...
        public void RequestGroupExpiredDueToNotFullyAnswered(RequestGroup requestGroup)
        {
            NullCheckHelper.ArgumentCheckNull(requestGroup, nameof(RequestGroupExpiredDueToNotFullyAnswered), nameof(NotificationService));
            var orderGroupNumber = requestGroup.OrderGroup.OrderGroupNumber;
            string frameworkAgreementNumber = requestGroup.Ranking.FrameworkAgreement.AgreementNumber;
            var email = GetOrganisationNotificationSettings(requestGroup.Ranking.BrokerId, NotificationType.RequestGroupLostDueToNotFullyAnswered, NotificationChannel.Email);
            if (email != null)
            {
                CreateEmail(email.ContactInformation,
                    $"Sammanhållen bokningsförfrågan {orderGroupNumber} har gått vidare till nästa förmedling i rangordningen",
                    $"Ni har inte namngivit tolk för den bekräftade sammanhållna bokningsförfrågan {orderGroupNumber} från {requestGroup.OrderGroup.CustomerOrganisation.Name} organisationsnummer {requestGroup.OrderGroup.CustomerOrganisation.OrganisationNumber}.\nTidsfristen enligt ramavtal har nu gått ut. {GoToRequestGroupPlain(requestGroup.RequestGroupId)}",
                    $"Ni har inte namngivit tolk för den bekräftade sammanhållna bokningsförfrågan {orderGroupNumber} från {requestGroup.OrderGroup.CustomerOrganisation.Name} organisationsnummer {requestGroup.OrderGroup.CustomerOrganisation.OrganisationNumber}.<br />Tidsfristen enligt ramavtal har nu gått ut. {GoToRequestGroupButton(requestGroup.RequestGroupId)}",
                    NotificationType.RequestGroupLostDueToNotFullyAnswered,
                    frameworkAgreementNumber,
                    true);
            }
            var webhook = GetOrganisationNotificationSettings(requestGroup.Ranking.BrokerId, NotificationType.RequestGroupLostDueToNotFullyAnswered, NotificationChannel.Webhook);
            if (webhook != null)
            {
                CreateWebHookCall(
                    new RequestGroupLostDueToInactivityModel
        ...
        public void ComplaintCreated(Complaint complaint)
        {
            ...
            string frameworkAgreementNumber = complaint.Request.Ranking.FrameworkAgreement.AgreementNumber;
...
{GoToRequestButton(complaint.Request.RequestId, HtmlHelper.ViewTab.Complaint)}",
        ...
        public void RequisitionCreated(Requisition requisition)
        {
            ...
            if (NotficationTypeAvailable(notificationType, NotificationConsumerType.Customer, NotificationChannel.Email) && !NotficationTypeExcludedForCustomer(notificationType))
            {
                ...
                var frameworkAgreementNumber = requisition.Request.Ranking.FrameworkAgreement.AgreementNumber;
...
{GoToWebHookListButton("Gå till systemets loggsida")}",
        ...
        private async Task<FlexibleRequestModel> GetFlexibleRequestModel(Request request)
        {
            var model = await GetRequestModel<FlexibleRequestModel>(request);
            model.ExpectedLength = request.Order.Duration;
            model.StartAt = null;
            model.EndAt = null;
            model.FlexibleStartAt = request.Order.StartAt;
            model.FlexibleEndAt = request.Order.EndAt;
            return model;
        }

        private async Task<T> GetRequestModel<T>(Request request) where T : RequestModel, new()
        {
            var order = await _dbContext.Orders.GetFullOrderById(request.OrderId);
            var priceRows = _priceCalculationService.GetPrices(request, _clock.SwedenNow, (CompetenceAndSpecialistLevel)order.PriceCalculatedFromCompetenceLevel, null, null).PriceRows.ToList();
